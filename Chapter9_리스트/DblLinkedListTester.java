...
public class DblLinkedListTester {
	static Scanner stdIn = new Scanner(System.in);
		...
		// 메뉴 열거형 
		enum Menu{
			ADD_FIRST("머리에 노드를 삽입"), ADD_LAST("꼬리에 노드를 삽입"), ADD("선택 노드의 바로 뒤에 삽입"), RMV_FIRST("머리 노드를 삭제"), RMV_LAST("꼬리 노드를 삭제"),
			RMV_CRNT("선택 노드를 삭제"), CLEAR("모든 노드를 삭제"), SEARCH_NO("번호로 검색"), SEARCH_NAME("이름으로 검색"), NEXT("선택 노드로 이동"), PREV("선택 노드를 앞쪽으로"),
		...
		static Menu SelectMenu() {
			int key;
			do {
				// ordinal() 메서드 : 해당 열거체 상수가 열거체 정의에서 정의된 순서(0부터)를 반환! 
				for(Menu m:Menu.values()) {
